...
bool AttinyManager::receiveUARTPacket(uint8_t* addr, uint8_t* buf, uint8_t* len, uint16_t bufsize, uint16_t timeout) {
...
void AttinyManager::startAttiny(uint8_t idx) {
    sendCmd(attinyAddresses_[idx], CMD_SEND_BNO_STATUS);
    if (!readBNOStatus(idx)) {
        Serial.printf("BNO konnte nicht gestartet werden: 0x%02X\n", attinyAddresses_[idx]);
    } else {
