...
void startAttiny(int idx) {
    ...
    sendCmd(attinyAddresses[idx], CMD_SEND_BNO_STATUS); // Chip-ID an alle Attinys
    Serial.println("Chip-ID Anfrage an Attiny 0x" + String(attinyAddresses[idx], HEX) + " gesendet.");
    doc["i"] = idx;
    doc["m"] = String("S: Init: ") + "0x" + String(attinyAddresses[idx], HEX);
    serializeJson(doc, Serial);
    Serial.println();

    if(!readBNOStatus())  
      Serial.println("BNO konnte nicht gestartet werden: 0x" + String(attinyAddresses[idx], HEX));
    delay(5);
}
...
void extractQuatsForBLE(const SensorData sensorData[], uint8_t quatData[NUM_ATTINYS][8]) {
  for (int i = 0; i < NUM_ATTINYS; ++i) {
    if (sensorData[i].valid && sensorData[i].dataLen >= 32) {
      memcpy(quatData[i], &sensorData[i].buffer[18], 8);
    } else {
